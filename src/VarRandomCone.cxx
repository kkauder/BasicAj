...
int main ( int argc, const char** argv ) {
  ...
  const char *defaults[5] = {"VarRandomCone","AjResults/VRC_Fresh_NicksList_HC100.root","AjResults/R0.2_Fresh_NicksList_HC100_AuAu.root","MB","Data/NewPicoDst_AuAuCentralMB/*.root"};
  // const char *defaults[5] = {"VarRandomCone","AjResults/VRC_Fresh_NicksList_HC100_ppInAuAu.root","AjResults/Tow0_Eff0_R0.2_Fresh_NicksList_HC100_ppInAuAuAj.root","MB","Data/NewPicoDst_AuAuCentralMB/*.root"};
  if ( argc==1 ) {
    argv=defaults;
    argc=5;
  }
  ...
  OrigJets->SetBranchAddress("jm1", &pJM1);
  TLorentzVector *pJM2 = new TLorentzVector();
  OrigJets->SetBranchAddress("jm2", &pJM2);
  ...
  for ( Long64_t OrigEv = 0; OrigEv< OrigJets->GetEntries() ; ++ OrigEv ){
    ...
    Current.push_back( MakePseudoJet( pJM1 ) );
    Current.push_back( MakePseudoJet( pJM2 ) );
  ...
  // Save results
  // ------------
  TTree* RCTree=new TTree("RCTree","Results from VarRandomCone");
  TLorentzVector jm1, jm2;
  RCTree->Branch("jm1",pJM1);
  RCTree->Branch("jm2",pJM2);

  float origaj_lo, modaj_lo;
  float origpt1, modpt1;
  float origpt2, modpt2;
  double refmult; // Really an int, but may change if using refmultcorr
  
  RCTree->Branch("refmult",&refmult, "refmult/D");

  RCTree->Branch("origaj_lo",&origaj_lo, "origaj_lo/F");
  RCTree->Branch("origpt1",&origpt1, "origpt1/F");
  RCTree->Branch("origpt2",&origpt2, "origpt2/F");

  RCTree->Branch("modaj_lo",&modaj_lo, "modaj_lo/F");
  RCTree->Branch("modpt1",&modpt1, "modpt1/F");
  RCTree->Branch("modpt2",&modpt2, "modpt2/F");
  ...
  fastjet::Selector GrabRing = fastjet::SelectorDoughnut( Rin, Rout );
  while ( reader.NextEvent() ) {
    reader.PrintStatus(10);      
    ...
    for ( vector<int>::iterator jit=JetIndices.begin(); jit!=JetIndices.end(); ++ jit ){
      PseudoJet& jm1 = vOrigJets.at(*jit).at(0);
      // GrabCone.set_reference( jm1);
      // vector<PseudoJet> C1 = GrabCone( pLo );
      GrabRing.set_reference( jm1);
      vector<PseudoJet> C1 = GrabRing( pLo );
      PseudoJet RC1 (0,0,0,0);
      for (vector<PseudoJet>::iterator it = C1.begin() ; it!= C1.end() ; ++ it ) RC1+= *it;
      
      PseudoJet& jm2 = vOrigJets.at(*jit).at(1);
      // GrabCone.set_reference( jm2);
      // vector<PseudoJet> C2 = GrabCone( pLo );
      GrabRing.set_reference( jm2);
      vector<PseudoJet> C2 = GrabRing( pLo );
      ...
      OrigAJ_hi->Fill( fabs ( (jm1.pt() - jm2.pt())  / (jm1.pt() + jm2.pt()) ), refmult );
      
      // float pt1 = jm1.pt() + RC1.pt() - TMath::Pi()*R*R * JAlo.GetBackgroundEstimator()->rho() ;
      // float pt2 = jm2.pt() + RC2.pt() - TMath::Pi()*R*R * JAlo.GetBackgroundEstimator()->rho() ;

      float pt1 = jm1.pt() + RC1.pt() - TMath::Pi()*(Rout*Rout - Rin*Rin) * JAlo.GetBackgroundEstimator()->rho() ;
      float pt2 = jm2.pt() + RC2.pt() - TMath::Pi()*(Rout*Rout - Rin*Rin) * JAlo.GetBackgroundEstimator()->rho() ;

      RandomAJ_lo->Fill( fabs ( (pt1 - pt2) / (pt1 + pt2) ), refmult );
      NoFabsRandomAJ_lo->Fill( (pt1 - pt2) / (pt1 + pt2), refmult );

      origaj_lo = ( jm1.pt() - jm2.pt())  / (jm1.pt() + jm2.pt());
