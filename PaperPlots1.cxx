...
void PaperPlots1() {
  ...
  // TFile *fAuAu         = TFile::Open("AjResults/rndm2/R0.2_Presel_AuAuAj.root");
  ...
  // // TFile *fAuAu         = TFile::Open("AjResults/testVerySmall.root");
  ...
  cerr << "Kolmogorov-Smirnov for pp @ AuAu vs. AuAu, HIGH cut: " << AJ_lo_sysstat->KolmogorovTest(AuAuAJ_lo, "") << endl;
  cerr << "chi^2 test for pp @ AuAu vs. AuAu, HIGH cut: " << AJ_lo_sysstat->Chi2Test(AuAuAJ_lo, "") << endl;
  cerr << "OLD: Kolmogorov-Smirnov for pp @ AuAu vs. AuAu, HIGH cut: " << ppInAuAuAJ_lo->KolmogorovTest(AuAuAJ_lo, "") << endl;
  cerr << "OLD: chi^2 test for pp @ AuAu vs. AuAu, HIGH cut: " << ppInAuAuAJ_lo->Chi2Test(AuAuAJ_lo, "") << endl;
  return;    
  ...
  if (TString(fAuAu->GetName()).Contains("R0.2") ){
    ...
    gPad->SaveAs("plots/Pt1_Fig1.png");
    gPad->SaveAs("plots/Pt1_Fig1.pdf");
  } else {
  ...
  sprintf ( plabel, "KS-value: %.2g", ppInAuAuAJ_lo->KolmogorovTest(AuAuAJ_lo, "") );
  latex.SetTextColor( AuAuAJ_lo->GetLineColor() );
  latex.DrawLatex( .6,.45, plabel);

  sprintf ( plabel, "p-value: %.2g", ppInAuAuAJ_hi->Chi2Test(AuAuAJ_hi, "") );
  latex.SetTextColor( AuAuAJ_hi->GetLineColor() );
  latex.DrawLatex( .6,.4, plabel);
  
  sprintf ( plabel, "p-value: %.2g", ppInAuAuAJ_lo->Chi2Test(AuAuAJ_lo, "") );
  latex.SetTextColor( AuAuAJ_lo->GetLineColor() );
  latex.DrawLatex( .6,.35, plabel);
  

  if (TString(fAuAu->GetName()).Contains("R0.2") ){
    gPad->SaveAs("plots/R0.2_Remake.png");
  } else if (TString(fAuAu->GetName()).Contains("Pt1") ){
    gPad->SaveAs("plots/Pt1_Remake.png");
  } else {
