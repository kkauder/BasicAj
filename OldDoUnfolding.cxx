...
int OldDoUnfolding(  TString trainname = "AjResults/TrainedWith_Groom_Aj_TrueMB_NoEff_GeantMc.root",
  ...
  TH2D* TestMeas2D  = (TH2D*) fmeas->Get("TrigTestMeas2D");

  // Load response
  // ------------
  TH2D* TrainMeas2D  = (TH2D*) ftrain->Get("TrigMeas2D");
  TH2D* TrainTruth2D  = (TH2D*) ftrain->Get("TrigTruth2D");

  RooUnfoldResponse* LeadPtResponse = (RooUnfoldResponse*) ftrain->Get("TrigPtResponse");
  ...
  RooUnfoldResponse* LeadPtZgResponse2D = (RooUnfoldResponse*) ftrain->Get("TrigPtZgResponse2D");

  const int MaxnBayes2D = 6;
  TH2D* BayesUnfolded[MaxnBayes2D-1];
  for ( int nBayes2D=1; nBayes2D<MaxnBayes2D; ++nBayes2D){
    ...
    BayesUnfolded[nBayes2D-1] = (TH2D*) BayesUnfold.Hreco();
    TString hname = "BayesUnfolded_"; hname+=nBayes2D;
    BayesUnfolded[nBayes2D-1] ->SetName(hname);
  }
  ...
  gPad->SetLogy();
  TLegend* leg = new TLegend( 0.55, 0.45, 0.89, 0.9, "Leading Jet p_{T}" );
  leg->SetBorderSize(0);
  leg->SetLineWidth(10);
  leg->SetFillStyle(0);
  leg->SetMargin(0.1);
  ...
  for ( int nBayes2D=1; nBayes2D<MaxnBayes2D; ++nBayes2D){
    TString hname = "unfoldpt_"; hname+=nBayes2D;
    TH1D* unfoldpt = (TH1D*) BayesUnfolded[nBayes2D-1]->ProjectionX(hname);
    // unfoldpt->SetLineColor(kGreen+1);
    unfoldpt->SetLineColor(nBayes2D+1);
    unfoldpt->Rebin( rebinpt );
    unfoldpt->Draw("same");
    TString title = "Unfolded, N = ";
    title +=nBayes2D;
    leg->AddEntry(unfoldpt->GetName(), title);
  }
