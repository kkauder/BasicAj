...
int CrossCheckForResAj_MatchGeantToPythia (
					   // TString PpLevelFile = "AjResults/CrossCheckForResAj_NoEff_NoCut_Geant.root"
					   // TString PpLevelFile = "AjResults/CrossCheckForResAj_HC0_NoEff_NoCut_TrueMB_HT54_HTled_Geant.root"
					   // TString PpLevelFile = "AjResults/CrossCheckForResAj_NoEff_NoCut_TrueMB_HT54_HTled_Geant.root"
					   TString PpLevelFile = "AjResults/CrossCheckForResAj_NoEff_NoCut_TrueMB_HT54_Geant.root"
					   ) {
  gStyle->SetOptStat(0);
  ...
  for ( Long64_t ppEvi = 0; ppEvi< PpJets->GetEntries() ; ++ppEvi ){
    ...
    TLorentzVector* PpT = (TLorentzVector*) PpTriggerLo->At(0);
    TLorentzVector* PpA = 0;
    if (PpAwayJet->GetEntries()!=0 ) PpA = (TLorentzVector*) PpAwayLo->At(0);
    ...
    if ( PpA && McA->DeltaR( *PpA ) < RCut ) MatchA = McA;
    else if ( PpA && McT->DeltaR( *PpA ) < RCut ) MatchA = McT;
    ...
    // Ignore the away side for this check
    // -----------------------------------
