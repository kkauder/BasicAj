{
  ...
  // ==============================================================================================
  TFile* fZZ = new TFile("AjResults/Groom_Y14_QNDlist_AuAu.root");
  int RefmultCut = 261;  // very rough guess for Run14 0-20%
  h2 = (TH2D*) fZZ->Get( "AJ_lo");
  int ZZMultBinL = h2->GetYaxis()->FindBin( RefmultCut );
  int ZZMultBinR = h2->GetNbinsY();

  TH1D* ZZhi = (TH1D*) ( (TH2D*) fZZ->Get( "AJ_hi") )->ProjectionX("ZZ_hi", ZZMultBinL, ZZMultBinR);
  ZZhi->SetName("ZZhi");
  ZZhi->SetLineColor( kGreen+2 );
  ZZhi->SetMarkerColor( kGreen+2 );
  ZZhi->SetMarkerStyle( 20 );

  TH1D* ZZlo = (TH1D*) ( (TH2D*) fZZ->Get( "AJ_lo") )->ProjectionX("ZZ_lo", ZZMultBinL, ZZMultBinR);
  ZZlo->SetName("ZZlo");
  ZZlo->SetLineColor( kGreen+2 );
  ZZlo->SetMarkerColor( kGreen+2 );
  ZZlo->SetMarkerStyle( 20 );

  TString ZZtitle = "Year 14 HT2 & HT3, No cuts, 0-20%";
  toa.Add(ZZlo);
  toa.Add(ZZhi);

  // ==============================================================================================
  TFile* fNN = new TFile("AjResults/Fresh_NicksList_HC100_AuAu.root");
  ...
  TString NNtitle = "Year 7 L2gamma, No cuts, 0-20%";
  toa.Add(NNlo);
  toa.Add(NNhi);
  ...
  sprintf ( plabel, "p-value = %0.2g", AAhi->KolmogorovTest(NNhi, "") );
  ...
  sprintf ( plabel, "p-value = %0.2g", DDhi->KolmogorovTest(NNhi, "") );
  leghi->AddEntry ( (TObject*) 0, plabel, "");
  ...
  leghi->AddEntry ( ZZhi, ZZtitle, "l");
  sprintf ( plabel, "p-value = %0.2g", ZZhi->KolmogorovTest(NNhi, "") );
  leghi->AddEntry ( (TObject*) 0, plabel, "");

  leghi->AddEntry ( NNhi, NNtitle, "l");
  sprintf ( plabel, "p-value = %0.2g", NNhi->KolmogorovTest(NNhi, "") );
  leghi->AddEntry ( (TObject*) 0, plabel, "");
  leghi->Draw();

  gPad->SaveAs("y14_Hi_Overlay.png");
  // ==============================================================================================
  ...
  sprintf ( plabel, "p-value = %0.2g", AAlo->KolmogorovTest(NNlo, "") );
  ...
  sprintf ( plabel, "p-value = %0.2g", DDlo->KolmogorovTest(NNlo, "") );
  leglo->AddEntry ( (TObject*) 0, plabel, "");
  ...
  leglo->AddEntry ( ZZlo, ZZtitle, "l");
  sprintf ( plabel, "p-value = %0.2g", ZZlo->KolmogorovTest(NNlo, "") );
  leglo->AddEntry ( (TObject*) 0, plabel, "");
  leglo->Draw();

  leglo->AddEntry ( NNlo, NNtitle, "l");
  sprintf ( plabel, "p-value = %0.2g", NNlo->KolmogorovTest(NNlo, "") );
  leglo->AddEntry ( (TObject*) 0, plabel, "");
  leglo->Draw();

  gPad->SaveAs("y14_Lo_Overlay.png");
  // ==============================================================================================
  
  cout << "Run11 NPE18, no cuts: " << AAhi->GetEntries() << endl;
  cout << "Run11 NPE18, all cuts: " << DDhi->GetEntries() << endl;
  cout << "Run7 L2gamma, no cuts: " << NNhi->GetEntries() << endl;
  cout << "Run14 HT2+HT3, no cuts: " << ZZhi->GetEntries() << endl;
